...
impl GamePlugin for Fallout4Plugin {
    ...
    fn analyze_archive(&self, game_root: &Path, entries: &[ArchiveEntry]) -> DeployPlan {
        ...
        if has_top_level_script_extender_files(&rel_paths) {
            // F4SE / ENB / proxy-DLL archives carry loader files at the root and
            // often bundle a Data/ folder too. Preserve the whole structure into
            // the game root so the loader .exe/.dll land at the root and Data/
            // lands in Data/ (merge_data would copy only Data/ and drop the loader).
            DeployPlan {
