...
export function AddToSteamPanel({ compact = false }: AddToSteamPanelProps) {
  ...
  const addToSteam = async () => {
    ...
    try {
      ...
      addToast(
        "Added to Steam",
        `Restart Steam, then launch "${result.display_name}" from your library.`,
        "success"
      );
    } catch (e) {
      addToast(
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        ...
        <p className="text-xs text-[var(--color-muted)]">
          Steam must be fully closed and reopened for the shortcut to appear. A backup of
          shortcuts.vdf is created automatically.
        </p>
      </CardContent>
    </Card>
  );
