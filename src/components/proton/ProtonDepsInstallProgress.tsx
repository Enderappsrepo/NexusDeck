  ...
  return (
    <div className="space-y-3 rounded-xl border border-[var(--color-border)] bg-[var(--color-secondary)]/30 p-4">
      <div className="space-y-2">
        ...
        {elapsedSec >= 30 && current?.status === "preparing" && (
          <p className="text-xs text-[var(--color-warning)]">
            Still preparing — detecting Protontricks and locating your prefix on the host. This is
            normal on first run.
          </p>
        )}
      </div>
      ...
      {activity.length > 0 && (
        <div className="rounded-lg bg-[var(--color-secondary)]/50 px-3 py-2 font-mono text-xs text-[var(--color-muted)]">
          {activity.map((line, i) => (
            <div key={`${line}-${i}`} className={i === activity.length - 1 ? "text-[var(--color-foreground)]" : ""}>
              {line}
            </div>
          ))}
        </div>
