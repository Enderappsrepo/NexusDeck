...
import { CheckCircle2, Circle, Copy, ExternalLink, Loader2, Terminal, Wrench, XCircle } from "lucide-react";
...
import { PROTON_DEPS_PACKAGES, type DepsVerification, type ProtontricksHealth, type ProtontricksInfo, type RepairStep } from "@/lib/autofix-types";
...
export function ProtontricksGuidePanel({
  ...
  const loggingActive = installing || repairing || generatingDiag;
  const { lines: protonLogLines, clear: clearProtonLog } = useProtonLogger(loggingActive);
  useWakeLock(installing || repairing, "Working on Protontricks");
  ...
  const repairProtontricks = async () => {
    setRepairing(true);
    setMessage("Running Protontricks repairs — this can take up to a minute. Keep NexusDeck open.");
    setLogPath(null);
    setRepairSteps([]);
    clearProtonLog();
    try {
      const result = await api.repairProtontricks();
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        ...
        {health && !health.healthy && (
          <div className="rounded-xl border border-[var(--color-warning)]/40 bg-[var(--color-warning)]/10 p-3 text-sm">
            ...
            <Button
              size="sm"
              className="mt-3"
              onClick={() => void repairProtontricks()}
              loading={repairing}
              disabled={repairing}
              data-focusable="true"
            >
        ...
        {repairSteps.length > 0 && (
          <ul className="space-y-1.5 rounded-xl border border-[var(--color-border)] p-3 text-sm">
            {repairSteps.map((s) => (
              <li key={s.name} className="flex items-start gap-2">
                ...
                <span className="min-w-0 flex-1">
                  ...
                  <span className="block text-xs text-[var(--color-muted)]">{s.detail}</span>
                </span>
              </li>
            ))}
          </ul>
